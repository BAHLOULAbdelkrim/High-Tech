...
// À ajouter dans main.js (ou un autre script exécuté sur toutes les pages)
document.addEventListener('DOMContentLoaded', function() {
  document.querySelectorAll('.lang-dropdown a, .lang-selector a[data-lang]').forEach(a => {
    a.addEventListener('click', function(e) {
      // extraire la première partie de l'href (ex: /fr/ -> 'fr')
      try {
        const href = a.getAttribute('href') || '';
        const m = href.match(/^\/(fr|en|ar)(\/|$)/i);
        if (m) {
          localStorage.setItem('preferredLang', m[1].toLowerCase());
        }
      } catch (err) {
        // ignore
      }
      // laisser le lien faire sa navigation normale
    });
  });
});
